...
/**
 * 修改自：https://github.com/jeasonlzy/PullZoomView
 */
public class PullZoomView extends ScrollView {
    ...
    public static int lastMaxY = 0;//解决meta10头部白条bug

    private float sensitive = 1.5f;         //放大的敏感系数
    private int zoomTime = 500;             //头部缩放时间，单位 毫秒
    private boolean isParallax = false;      //是否让头部具有视差动画
    ...
    public PullZoomView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                maxY = contentView.getTop() - lp.topMargin;//只有布局完成后才能获取到正确的值
                if (maxY == 0) {
                    maxY = lastMaxY;
                }
                lastMaxY = maxY;
    ...
    public void initView(View headerView, View zoomView, View contentView) {
...
//        if (scrollListener != null && isScrolledToBottom()) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                isActionDown = false;
                if (isZooming || headerHeight < headerParams.height) {
